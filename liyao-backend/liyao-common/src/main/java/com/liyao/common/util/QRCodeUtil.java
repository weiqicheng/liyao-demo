...
public final class QRCodeUtil extends LuminanceSource {
     ...
     * @throws IOException
     */
    public static InputStream writeToInputstream(BitMatrix matrix,String format) throws IOException {
        BufferedImage image = toBufferedImage(matrix);
     ...
     * @throws Exception
     */
    public static InputStream generateQRCodeToInputStream(String text, int width, int height, String format)
            throws Exception {
        Hashtable<EncodeHintType, Object> hints = new Hashtable<EncodeHintType, Object>();
        hints.put(EncodeHintType.CHARACTER_SET, "utf-8");// 指定编码格式
        hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);// 指定纠错等级
        hints.put(EncodeHintType.MARGIN, 1); // 白边大小，取值范围0~4
        BitMatrix bitMatrix = new MultiFormatWriter().encode(text, BarcodeFormat.QR_CODE, width, height, hints);
        InputStream inputStream = writeToInputstream(bitMatrix,format);
        return inputStream;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            // 生成二维码图片，并返回图片路径
            String pathName = generateQRCode(text, width, height, format, "/Users/gongmb/java//test/test.png");
